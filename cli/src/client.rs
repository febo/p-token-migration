...
pub async fn start_client(
    ...
    send_transaction(
        ...
        &instructions,
        &payer.pubkey(),
        &[&account_a, &account_b, &payer, &mint, &mint_authority],
    )
    .await
    .unwrap();
    */
...
pub async fn start_monitor(
    ...
    loop {
        ...
        /*
        if let Ok(signature) = send_transaction(
            &rpc_client,
            &instructions,
            &payer.pubkey(),
            &[&payer, &authority],
        ...
        .await
        {
            ...
            loop {
                ...
                progress_bar.set_message("🖥️  CU (transfer): -");
            ...
            if let Some(meta) = status.transaction.meta {
                if let OptionSerializer::Some(units) = meta.compute_units_consumed {
                    progress_bar.set_message(format!("🖥️  CU (transfer): {units}"));
                }
            }
        } else {
